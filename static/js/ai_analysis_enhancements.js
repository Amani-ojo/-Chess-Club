 ...
 *
 * This file is additive — it reads data from hidden script/pre elements that
 * the template emits and renders three new sections:
 *   1. Eval line chart
 *   2. Critical-moment cards (mini boards with played vs. best move)
 *
 * It does NOT modify or read from any existing JS globals. If the expected
 * containers are absent (e.g. analysis not completed) it silently does nothing.
 */
(function () {
    'use strict';
    ...
    /* Build FEN-before-each-ply array by replaying the PGN through chess.js */
    ...
    function pickCriticalIndices(moves) {
        // Worst moves by mover's-perspective eval delta. Threshold: ≥1 pawn drop.
    ...
    function narrationFor(move, before, after, isWhite) {
            ...
            col.innerHTML =
                '<div class="card border-0 shadow-sm h-100">' +
                    ...
                    '<div class="card-footer bg-light small">' +
                        narrationFor(m, m.eval_before, m.eval_after, m.is_white) +
                    '</div>' +
                '</div>';
