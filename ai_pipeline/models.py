...
class Game(models.Model):
    """
    A game fetched from Lichess or played through the club.
    Stores the PGN and links to both players.
    """
    lichess_game_id = models.CharField(max_length=20, unique=True, blank=True, null=True)
    player_white = models.ForeignKey(
        Member, on_delete=models.CASCADE, related_name='lichess_games_as_white'
    )
    player_black = models.ForeignKey(
        Member, on_delete=models.CASCADE, related_name='lichess_games_as_black'
    )
...
class GameAnalysis(models.Model):
    """
    Stockfish analysis summary for an entire game.
    Created by the Celery analysis worker.
    """
    STATUS_CHOICES = [
        ('pending', 'Pending'),
...
class MoveEvaluation(models.Model):
    """
    Per-move Stockfish evaluation within a game.
    """
    ...
    analysis = models.ForeignKey(
        GameAnalysis, on_delete=models.CASCADE, related_name='move_evaluations'
    )
    ...
    classification = models.CharField(
        max_length=20, choices=CLASSIFICATION_CHOICES, default='good'
    )
...
class PlayerInsight(models.Model):
    """
    Aggregated weakness patterns and recommendations for a member,
    built from multiple game analyses over time.
    """
    CATEGORY_CHOICES = [
        ('opening', 'Opening'),
    ...
    member = models.ForeignKey(
        Member, on_delete=models.CASCADE, related_name='insights'
    )
